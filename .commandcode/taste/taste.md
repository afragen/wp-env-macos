...
- Reports bugs and debugging follow-ups tersely, supplying only the key narrowing fact (e.g., "it does work for latest", or "but `.env` has `WP_ENV_CACHE_DIR=...7.0.3` set") and expecting the agent to investigate to root cause, fix, and verify rather than asking clarifying questions. Confidence: 0.75
...
- When a package ships both a primary command name and a working alias (both bins installed), prefers docs to default every command, heading, and piece of prose to the primary name while keeping a single accurate alias note (e.g., "`wp-env-macos` (alias `mac-env`)") rather than purging the alias mention entirely. Confidence: 0.9
